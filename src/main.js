...
$('#dataBtn').onclick=()=>{
  ...
  renderModalLog();
  if(typeof updateBackupUi==='function'){ updateBackupUi(AutoBackup && AutoBackup.getStatus ? AutoBackup.getStatus() : {code:'unsupported'}); }
};
...
function normalizeImportedState(parsed){
  ...
  if(Number.isFinite(Number(source.round))) out.round = Math.max(1, Math.round(Number(source.round)));
  else if(source.rounds && source.rounds.current && Number.isFinite(Number(source.rounds.current.number))) out.round = Math.max(1, Math.round(Number(source.rounds.current.number)));
  else out.round = 1;
  ...
  if(source.rounds && Array.isArray(source.rounds.history)){
    ...
    out.roundWinners = source.roundWinners.map(r=>{
      if(typeof r === 'string'){
        if(r === 'A') return { winner: 'A', scoreA: 0, scoreB: 0 };
        if(r === 'B') return { winner: 'B', scoreA: 0, scoreB: 0 };
        return { winner: 'T', scoreA: 0, scoreB: 0 };
...
function setupServiceWorkerUpdates(){
  ...
  function requestRefresh(){
    if(waitingWorker){
      ...
      localStorage.setItem('punchBuggy_running_version', window.PUNCHBUGGY_APP_VERSION || 'unknown');
      console.log('[PunchBuggy] Update requested, new version will be:', window.PUNCHBUGGY_APP_VERSION);
      hideBanner();
